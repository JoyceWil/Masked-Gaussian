...
def training(
    ...
    # Set up dataset
    # 假设 Scene 类已经按照第一部分的说明被修改，可以加载 S_map
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # >>>>> 开始修改: S_map 加权损失 <<<<<
        # 计算 L1 损失
        if opt.use_s_map_loss and hasattr(viewpoint_cam, 's_map') and viewpoint_cam.s_map is not None:
            s_map = viewpoint_cam.s_map.cuda()

            # 稳健的权重策略: 基础权重为1, S_map提供额外加成
            # 这样可以避免非重要区域梯度为0
            weight_map = 1.0 + opt.lambda_s_map * s_map

            # 计算加权L1损失
            pixel_wise_l1_loss = torch.abs(image - gt_image)
            render_loss = (weight_map * pixel_wise_l1_loss).mean()

            # (可选) 记录一下未加权的损失用于对比
            with torch.no_grad():
                loss["l1_unweighted"] = pixel_wise_l1_loss.mean().item()
        else:
            # 如果不使用S_map，则退回标准的L1损失
            render_loss = l1_loss(image, gt_image)

        loss["render"] = render_loss
        loss["total"] += loss["render"]

        # SSIM 损失保持不变，因为它是一个结构性指标
        ...
        with torch.no_grad():
            ...
            metrics = {}
            for l in loss:
                metrics["loss_" + l] = loss[l] if isinstance(loss[l], float) else loss[l].item()
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    gt_image = viewpoint.original_image.to("cuda" if torch.cuda.is_available() else "cpu")
                    images.append(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_s_map_loss", action="store_true", help="启用 S_map 加权损失")
    ...
    # Ensure all args are propagated to opt
    for k, v in vars(args).items():
        if not hasattr(opt, k):
            setattr(opt, k, v)

    # 将新的 S_map 参数也传递给 opt
    if 'use_s_map_loss' in args_dict:
        opt.use_s_map_loss = args_dict['use_s_map_loss']
    if 'lambda_s_map' in args_dict:
        opt.lambda_s_map = args_dict['lambda_s_map']
    ...
    if not model_path_was_specified:
        if hasattr(args, 'source_path') and args.source_path:
            ...
            # Propagate path to all parameter groups
    ...
    if opt.use_s_map_loss:
        print(f"S_map加权损失已启用。强度 (lambda_s_map): {opt.lambda_s_map}")
